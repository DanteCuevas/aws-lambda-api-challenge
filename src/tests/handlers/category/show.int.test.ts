...
import * as showCategory from '../../../handlers/category/show';
...
describe('Show category integration tests', () => {
  ...
  test('it should return 200 with valid token', async () => {
    ...
    // @ts-ignore
    const res = await showCategory.handler(event);
    const response = JSON.parse(res.body);

    expect(res.statusCode).toBe(200);
    expect(response.category._id).toBeDefined();
    expect(response.category.name).toEqual(data.name);
    expect(response.category.description).toEqual(data.description);
